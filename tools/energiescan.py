...
def main_html():
    get = ''.join(f'<div><h3>{esc(h)}</h3><p>{esc(t)}</p></div>' for h, t in [
  ...
  {CSS}
  <div class="wrap reveal" style="max-width:1040px;padding-top:48px;">
    <div class="es-hero">
      <div>
        <div class="pill">Gemeente Moerdijk · {esc(DATES)}</div>
        <h1 class="vw-heading" style="font-size:clamp(32px,4.8vw,48px);margin-top:14px;line-height:1.1;">Gratis Energiescanweek Moerdijk</h1>
        <p style="font-size:17px;color:var(--ink-soft);margin-top:16px;line-height:1.65;">Twijfel je over zonnepanelen, een thuisbatterij of een warmtepomp? Of vraag je je af wat het einde van het salderen in 2027 voor jou betekent? Van {esc(DATES)} komen we gratis bij je langs voor eerlijk advies. Zonder verkooppraatje en zonder verplichtingen.</p>
        <div class="es-date">📅 <span>Ma 26 t/m za 31 oktober · <b>ook 's avonds en op zaterdag</b></span></div>
        <div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:24px;"><a href="#aanmelden" class="btn-primary" style="text-decoration:none;">{'Bekijk de status' if MODE == 'vol' else 'Meld je gratis aan'} →</a></div>
        <p style="font-size:13px;color:var(--ink-faint);margin-top:14px;">Voor inwoners van Zevenbergen, Klundert, Fijnaart, Willemstad en de andere kernen van de gemeente Moerdijk. Maximaal {MAX} huishoudens.</p>
      </div>
      <div class="img"><img fetchpriority="high" src="/images/monteur-en-klant.webp" alt="Adviseur van Voltwijk in gesprek met een bewoner" width="784" height="1080"></div>
    </div>
  </div>
  <div class="wrap reveal es-sec" style="max-width:1040px;">
    <h2 class="vw-heading es-h2">Wat je krijgt</h2>
    <div class="es-get">{get}</div>
  </div>
  <div class="wrap reveal es-sec" style="max-width:1040px;">
    <h2 class="vw-heading es-h2">Zo werkt het</h2>
    <div class="es-steps">
      ...
      <div><div class="step-num">3</div><p><b>Bezoek en advies</b>Een adviseur uit ons eigen team komt ±45 minuten langs. Daarna krijg je je adviesrapport per mail.</p></div>
    </div>
  </div>
  <div class="wrap reveal es-sec" id="aanmelden" style="max-width:1040px;scroll-margin-top:90px;">
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:40px;align-items:start;">
      ...
      <div>
        ...
        <p class="es-kernen">{esc(', '.join(KERNEN[:-1]))} en {esc(KERNEN[-1])}.</p>
      </div>
    </div>
  </div>
